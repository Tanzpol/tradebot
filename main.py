    ...
    try{
      ...
      for(const t of arr){
        html+=`<tr>
          ...
          <td>${t.status}</td>
          <td>${t.symbol} ${t.grid_part?('(сетка p'+t.grid_part+')'):''}</td>
...
<style>
  ...
  <div class="card">
    <table>
      ...
      <tr><th>Часть сетки</th><td id="gpart">—</td></tr>
      <tr><th>Количество</th><td id="qty">—</td></tr>
      <tr><th>Вход (лимит), $</th><td id="entry">—</td></tr>
      <tr><th>TP, $</th><td id="tp">—</td></tr>
      <tr><th>SL, $</th><td id="sl">—</td></tr>
...
async function loadTrade(){
  ...
  gpartEl.textContent = trade.grid_part ? ('p'+trade.grid_part) : '—';
  qtyEl.textContent = fmt(trade.qty, 8);
  entryEl.textContent = fmt(trade.entry_price, 2);
  tpEl.textContent = trade.tp_price ? fmt(trade.tp_price,2) : '—';
  slEl.textContent = trade.sl_price ? fmt(trade.sl_price,2) : '—';
...
def api_trades_grid():
    ...
    Уровни входа: 0%, -1%, -2% от текущей цены.
    Для каждой: TP=+1.5%, SL=-1.0% от цены входа.
    """
    try:
        body = request.get_json(force=True)
        symbol = (body.get("symbol") or DEFAULT_SYMBOL).upper()
        ...
        entry_mults = [1.00, 0.998, 0.994]  # более плавные уровни для высоких цен
        tp_mult = 1.015
        sl_mult = 0.99

        ref_price = price_public(symbol)
        out = []
        for idx, pshare in enumerate(parts, start=1):
            amt = round(total_usdc * pshare, 2)
            planned_entry = round(ref_price * entry_mults[idx-1], 2)
            qty = (amt / planned_entry) * (1 - FEE_RATE_BNB)
            spent = planned_entry * qty * (1 + FEE_RATE_BNB)
            tp_price = round(planned_entry * tp_mult, 2)
            sl_price = round(planned_entry * sl_mult, 2)

            t = add_trade(
                symbol, amt, round(planned_entry, 2), round(qty, 8), round(spent, 2),
                entry_mode="limit", planned_entry=round(planned_entry,2),
                tp_price=tp_price, sl_price=sl_price,
                grid_part=idx, grid_total=total_usdc
            )
            out.append(t)
...
def api_bot_status():
    """Возвращает состояние бота для интерфейса"""
    if trade_bot:
        return jsonify(trade_bot.status())
